// src/pages/guest/ReaderDashboard.tsx
...
const icons = {
  ...
  book:      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>,
...
function StatCard({ label, value, accent, icon }: StatCardProps) {
  return (
    <div style={{ flex: "1 1 180px", background: "#fff", border: `1.5px solid ${accent}33`, borderRadius: 12, padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <div>
...
function NavItem({ label, active, icon, onClick }: NavItemProps) {
  return (
    <button onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 13.5, fontWeight: active ? 600 : 400, background: active ? PALETTE.burntOrange : "transparent", color: active ? "#fff" : PALETTE.blushCream, transition: "background 0.18s, color 0.18s" }}
...
// ── Borrowed row ──────────────────────────────────────────────────────────────
function BorrowedRow({ br, onReturn }: { br: BorrowRecordPublicDto; onReturn: (id: string) => void }) {
  ...
  const handleReturn = async () => {
    ...
    try {
      ...
      await api.put(`/borrow-records/${br.borrowRecordId}`, { actualReturnDate: today });
      onReturn(br.borrowRecordId);
    } catch {
      setReturning(false);
    }
  };
  ...
  return (
    <div style={{ background: "#f0faf7", border: "1px solid #d4f0e8", borderRadius: 10, padding: "14px 18px", display: "flex", alignItems: "center", gap: 14 }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <p style={{ margin: "3px 0 0", fontSize: 12, color: PALETTE.slateGrey }}>
          Due: {new Date(br.dueDate).toLocaleDateString()}
        </p>
      </div>
      <button onClick={handleReturn} disabled={returning} style={{
        ...
        fontSize: 13, fontWeight: 500, padding: "7px 18px", borderRadius: 7,
        cursor: returning ? "not-allowed" : "pointer", opacity: returning ? 0.7 : 1,
      }}
        onMouseEnter={e => { if (!returning) e.currentTarget.style.opacity = "0.85"; }}
        onMouseLeave={e => { e.currentTarget.style.opacity = returning ? "0.7" : "1"; }}
      >
  ...
  return (
    <div style={{ minHeight: "100vh", background: PALETTE.blushCream, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{ background: PALETTE.darkNavy, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 54, position: "sticky", top: 0, zIndex: 100, gap: 12 }}>
        ...
        <button onClick={handleLogout} style={{ display: "flex", alignItems: "center", gap: 6, background: "transparent", border: "none", color: PALETTE.blushCream, fontFamily: "'DM Sans', sans-serif", fontSize: 13.5, cursor: "pointer", padding: "8px 4px", opacity: 0.8, flexShrink: 0 }}>
          {icons.logout} Logout
        </button>
      </nav>
      ...
      <main style={{ padding: "32px 40px", maxWidth: 1300, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13.5, color: PALETTE.slateGrey }}>Here's your reading activity</p>
        </div>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 32 }}>
          ...
          <StatCard
            ...
            accent="#e05a5a" icon={icons.clock}
          />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: "24px 28px", border: "1.5px solid #ede5e0" }}>
            ...
            {loading ? (
              <p style={{ color: PALETTE.slateGrey, fontSize: 13.5 }}>Loading…</p>
            ) : activeBorrows.length === 0 ? (
              <p style={{ color: PALETTE.slateGrey, fontSize: 13.5 }}>No books currently borrowed.</p>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {activeBorrows.map(br => (
                  <BorrowedRow key={br.borrowRecordId} br={br} onReturn={handleReturn} />
                ))}
              </div>
            )}
          </div>
