...
function WriteReviewForm({ onSubmit, onCancel }: {
  onSubmit: (rating: number, comment: string) => void; onCancel: () => void;
}) {
    ...
    <div style={{ background: "#f5faf8", border: "1px solid #ddf0e9", borderRadius: 10, padding: "20px 24px", marginBottom: 16 }}>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={() => { if (rating && comment.trim()) onSubmit(rating, comment); }}
          style={{ background: PALETTE.mintTeal, border: "none", color: PALETTE.darkNavy,
        ...
        <button onClick={onCancel}
          style={{ background: "transparent", border: "1.5px solid #ccc", color: PALETTE.slateGrey,
...
export default function BookDetailPage({ book, onBack, onNavigate }: BookDetailPageProps) {
  ...
  if (!book) {
    return (
      <div style={{ minHeight: "100vh", background: PALETTE.blushCream, display: "flex",
  ...
  const handleSubmitReview = (rating: number, comment: string) => {
    setReviews([{
      id: reviews.length + 1, name: "You",
      date: new Date().toLocaleDateString("en-US"), rating, comment,
    }, ...reviews]);
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: "24px 28px", border: "1.5px solid #ede5e0" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
              ...
              <h2 style={{ margin: 0, fontFamily: "'Playfair Display', serif",
                fontSize: 17, fontWeight: 700, color: PALETTE.darkNavy }}>Reader Reviews</h2>
              {!showReviewForm && (
            ...
            {showReviewForm && (
              <WriteReviewForm onSubmit={handleSubmitReview} onCancel={() => setShowReviewForm(false)} />
            )}
